...
options(survey.lonely.psu = "adjust")  # handle strata with single PSU after subsetting
...
df$age_cat      <- factor(
  ifelse(df$Q1 %in% 1:3, "<=14",
  ifelse(df$Q1 == 4, "15",
  ifelse(df$Q1 == 5, "16",
  ifelse(df$Q1 == 6, "17",
  ifelse(df$Q1 == 7, "18+", NA))))),
...
outcome_defs <- list(
  ...
  list(qn = "QN33", label = "Current cigarette use"),
  list(qn = "QN36", label = "Current e-cigarette use"),
  list(qn = "QN42", label = "Current alcohol use"),
  list(qn = "QN48", label = "Current marijuana use")
)
...
# ── 2. Weighted prevalence (survey-adjusted) ─────────────────
cat("Computing weighted prevalences...\n")

# Survey design for prevalence estimates
des_full <- svydesign(ids = ~psu, strata = ~stratum, weights = ~weight,
                      data = df, nest = TRUE)
...
for (od in outcome_defs) {
  ...
  for (lev in c("Never", "Rarely", "Sometimes", "Most", "Always")) {
    des_sub <- subset(des_full, !is.na(racism_f) & racism_f == lev)
    n_obs <- sum(!is.na(df[[col]]) & !is.na(df$racism_f) & df$racism_f == lev)
    if (n_obs < 5) {
      prev_rows[[length(prev_rows) + 1]] <- data.frame(
        outcome = od$label, outcome_var = col, racism_level = lev,
        pct = NA, lo = NA, hi = NA, n = 0, stringsAsFactors = FALSE)
      next
    }
    est <- svymean(as.formula(paste0("~", col)), des_sub, na.rm = TRUE)
    p <- coef(est)[1]
    se <- SE(est)[1]
    prev_rows[[length(prev_rows) + 1]] <- data.frame(
      outcome = od$label, outcome_var = col, racism_level = lev,
...
# Non-White prevalences (Panel B)
des_nw <- subset(des_full, !is.na(racism_f) & !is.na(raceeth) & raceeth != 5)
nw_prev_rows <- list()
for (od in outcome_defs) {
  col <- paste0(od$qn, "_bin")
  for (lev in c("Never", "Rarely", "Sometimes", "Most", "Always")) {
    des_sub <- subset(des_nw, racism_f == lev)
    n_obs <- sum(!is.na(des_sub$variables[[col]]))
    if (n_obs < 5) {
      nw_prev_rows[[length(nw_prev_rows) + 1]] <- data.frame(
        outcome = od$label, outcome_var = col, racism_level = lev,
        pct = NA, lo = NA, hi = NA, n = 0, stringsAsFactors = FALSE)
      next
    }
    est <- svymean(as.formula(paste0("~", col)), des_sub, na.rm = TRUE)
    p <- coef(est)[1]; se <- SE(est)[1]
    nw_prev_rows[[length(nw_prev_rows) + 1]] <- data.frame(
      outcome = od$label, outcome_var = col, racism_level = lev,
      pct = 100 * p,
      lo  = 100 * max(0, p - 1.96 * se),
...
nw_prev_df <- do.call(rbind, nw_prev_rows)
rownames(nw_prev_df) <- NULL
write.csv(nw_prev_df, file.path(DATA_DIR, "nw_prevalences.csv"), row.names = FALSE)

# 3-group suicide text stats (for Results paragraph)
sui_col <- "QN27_bin"
grp_never <- subset(des_full, !is.na(racism_f) & racism_f == "Never")
grp_rs    <- subset(des_full, !is.na(racism_f) & racism_f %in% c("Rarely", "Sometimes"))
grp_ma    <- subset(des_full, !is.na(racism_f) & racism_f %in% c("Most", "Always"))
sui_stats <- data.frame(
  group = c("Never", "Rarely/Sometimes", "Most/Always"),
  pct = 100 * c(coef(svymean(~QN27_bin, grp_never, na.rm = TRUE))[1],
                 coef(svymean(~QN27_bin, grp_rs, na.rm = TRUE))[1],
                 coef(svymean(~QN27_bin, grp_ma, na.rm = TRUE))[1]),
  stringsAsFactors = FALSE
)
write.csv(sui_stats, file.path(DATA_DIR, "suicide_text_stats.csv"), row.names = FALSE)
...
run_one_outcome <- function(outcome_col, label) {
  ...
  sub <- df[complete.cases(df[, c(outcome_col, "racism_f", "weight", "stratum", "psu",
  ...
  z <- tryCatch(
    zelig2(fml, model = "logit", data = sub, weights = ~weight,
           ids = ~psu, strata = ~stratum, nest = TRUE, num = NUM_SIMS),
  ...
  for (lev in c("Rarely", "Sometimes", "Most", "Always")) {
    z2 <- setx(z, racism_f = "Never", fn = "mean", factor_default = "mean")
    z2 <- setx1(z2, racism_f = lev, fn = "mean", factor_default = "mean")
    z2 <- sim(z2)
...
for (grp in c("Non-White", "White", "Black", "Hispanic", "Asian",
  ...
  for (od in outcome_defs) {
    ...
    base_vars <- c(outcome_col, "racism_f", "weight", "stratum", "psu",
                   "female", "age_cat", "english_prof", "good_grades")
    ...
    z <- tryCatch(
      zelig2(fml, model = "logit", data = sub2, weights = ~weight,
             ids = ~psu, strata = ~stratum, nest = TRUE, num = NUM_SIMS),
      error = function(e) {
...
for (grp in c("All", "Non-White", "White", "Black", "Hispanic", "Asian",
  ...
  for (od in outcome_defs) {
    ...
    base_vars <- c(outcome_col, "racism_3", "weight", "stratum", "psu",
                   "female", "age_cat", "english_prof", "good_grades")
    ...
    for (lev in c("Rarely/Sometimes", "Most/Always")) {
      sim_res <- tryCatch({
        z2 <- setx(z, racism_3 = "Never", fn = "mean", factor_default = "mean")
        z2 <- setx1(z2, racism_3 = lev, fn = "mean", factor_default = "mean")
        z2 <- sim(z2)
        qi <- zelig2_qi_to_df(z2)
        rr_draws <- qi$rr[is.finite(qi$rr)]
        if (length(rr_draws) >= 10) {
...
# Also save racism prevalence by race for text (survey-adjusted)
df$any_racism <- as.numeric(df$racism_f != "Never")
df$most_always <- as.numeric(df$racism_f %in% c("Most", "Always"))
des_racism <- svydesign(ids = ~psu, strata = ~stratum, weights = ~weight,
                        data = df[!is.na(df$racism_f), ], nest = TRUE)
...
for (race in c("Overall", "Asian", "Black", "Multiple", "Hispanic", "White")) {
  if (race == "Overall") {
    ...
    des_sub <- subset(des_racism, !is.na(race_cat) & race_cat == race)
  }
  n_obs <- nrow(des_sub$variables)
  est_any <- svymean(~any_racism, des_sub, na.rm = TRUE)
  est_ma  <- svymean(~most_always, des_sub, na.rm = TRUE)
  p_any <- coef(est_any)[1]; se_any <- SE(est_any)[1]
  p_ma  <- coef(est_ma)[1]
  any_rows[[length(any_rows) + 1]] <- data.frame(
    group = race, n = n_obs,
    any_pct = 100 * p_any,
    any_lo  = 100 * max(0, p_any - 1.96 * se_any),
    any_hi  = 100 * min(1, p_any + 1.96 * se_any),
    ma_pct  = 100 * p_ma,
    stringsAsFactors = FALSE
  )
}
